...
class Seq2SeqModel():
    ...
    def build_model(self):
        ...
        #=================================1, 定义模型的placeholder
        self.keyword = tf.placeholder(tf.int32, [None, None], name="keyword")
        self.keyword_length = tf.placeholder(tf.int32, [None], name="keyword_length")
        ...
        #=================================2, 定义模型的encoder部分
        # keyword_encoder        
        with tf.variable_scope("keyword_encoder"):
            encoder_cell = self._create_rnn_cell()
            embedding = np.load('embedding_matrix.npy')
            embedding=tf.get_variable(name="embedding",shape=embedding.shape,initializer=tf.constant_initializer(embedding), trainable=False)
            #embedding = tf.get_variable('embedding', [self.vocab_size, self.embedding_size])
            keyword_encoder_inputs_embedded = tf.nn.embedding_lookup(embedding, self.keyword)
            encoder_outputs, encoder_state = tf.nn.dynamic_rnn(encoder_cell, keyword_encoder_inputs_embedded,
                                                               sequence_length=self.keyword_length,
                                                               dtype=tf.float32)
            _, bi_states = tf.nn.bidirectional_dynamic_rnn( cell_fw = encoder_cell,
                                                            cell_bw = encoder_cell,
                                                            inputs = keyword_encoder_inputs_embedded,
                                                            sequence_length = self.keyword_length,
                                                            dtype = tf.float32, )

            self.keyword_state = tf.concat(bi_states, axis = 1)
            #self.keyword_state = bi_states
            #self.keyword_state = tf.concat(tf.cast(bi_states, tf.float32), axis = 1)
            #self.keyword_state = tf.cast(tf.concat(bi_states, axis = 1), tf.float32)
        ...
        with tf.variable_scope("context_encoder"):
            ...
            embedding = np.load('embedding_matrix.npy')
            embedding=tf.get_variable(name="embedding",shape=embedding.shape,initializer=tf.constant_initializer(embedding), trainable=False)
            context_encoder_inputs_embedded = tf.nn.embedding_lookup(embedding, self.context)
            bi_outputs, _ = tf.nn.bidirectional_dynamic_rnn(cell_fw = encoder_cell,
                                                            cell_bw = encoder_cell,
                                                            inputs = context_encoder_inputs_embedded,
                                                            sequence_length = self.context_length,
                                                            dtype = tf.float32,)
            #self.context_outputs = tf.cast(tf.concat(bi_outputs, axis = 2), tf.float32)
            #self.context_outputs = bi_outputs
        ...
        with tf.variable_scope('decoder'):
            ...
            if self.beam_search:
                ...
                keyword_length = tf.contrib.seq2seq.tile_batch(self.keyword_length, multiplier=self.beam_size)
                context_length = tf.contrib.seq2seq.tile_batch(self.context_length, multiplier=self.beam_size)

            #定义要使用的attention机制。

            attention_mechanism = tf.contrib.seq2seq.BahdanauAttention(num_units=self.rnn_size, memory=self.context_outputs,
                                                                     memory_sequence_length=context_length)
            ...
            decoder_initial_state = decoder_cell.zero_state(batch_size=batch_size, dtype=tf.float32).clone(cell_state=self.keyword_state)
